...
.run(function($templateCache) {
  $templateCache.put('udt-toolbar.html', '<div name="udt-toolbar" class="row margin-bottom-3"><div class="col-md-12 col-lg-12">'
					...
					+ '</div>'
  		    		...
  		    		+'</div>'
					+ '<div class="col-lg-5" ng-if="udtTable.config.search.showLocalSearch === true">'
					+  '<div class="row">'
					+  '<div class="col-lg-3">'
					+   '<input class="form-control" type="text" ng-model="searchTerms.$">'
					+  '</div>'
					+	'<button ng-if="udtTable.config.search.showLocalSearch === true" class="btn btn-default" ng-click="udtTable.searchLocal(searchTerms)" title="{{udtTableFunctions.messagesDatatable(\'datatable.button.searchLocal\')}}">'
  		    		+		'<i class="fa fa-search"></i>'
  		    		+		'<span ng-if="!udtTable.isCompactMode()"> {{udtTableFunctions.messagesDatatable(\'datatable.button.searchLocal\')}}</span>'
  		    		+	'</button>'	
					+ '</div>'
					+  '</div>'
